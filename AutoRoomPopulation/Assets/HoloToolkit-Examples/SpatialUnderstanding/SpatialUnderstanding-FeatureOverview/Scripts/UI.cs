...
public class UI : LineDrawer
{
    ...
    private void SetupMenus()
    {
        ...
        AddButton("On Floor", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnFloor(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("On Wall", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnWall(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("On Ceiling", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnCeiling(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("On SurfaceEdge", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnEdge(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("On FloorAndCeiling", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnFloorAndCeiling(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("RandomInAir AwayFromMe", Panels.LevelSolver, () => { LevelSolver.Instance.Query_RandomInAir_AwayFromMe(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("OnEdge NearCenter", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnEdge_NearCenter(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("OnFloor AwayFromMe", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnFloor_AwayFromMe(testObject); timeLastQuery = DateTime.MinValue; });
        AddButton("OnFloor NearMe", Panels.LevelSolver, () => { LevelSolver.Instance.Query_OnFloor_NearMe(testObject); timeLastQuery = DateTime.MinValue; });
